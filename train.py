...
from meldataset_0217 import MelDataset, mel_spectrogram, get_dataset_filelist_custom
from model_0217 import Generator, MultiPeriodDiscriminator, MultiScaleDiscriminator, feature_loss, \
    generator_loss, discriminator_loss
from utils import plot_spectrogram, scan_checkpoint, load_checkpoint, save_checkpoint, scale_normalize
...
def train(rank, a, h):
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y = torch.autograd.Variable(y.to(device, non_blocking=True))  # [Batch , T], Full resolution
            ...
            if rank == 0:
                ...
                if steps % a.validation_interval == 0:  # and steps != 0:
                    ...
                    with torch.no_grad():
                        for j, batch in enumerate(validation_loader):  # discriminator 통과안하고 mel loss만
                            x, y, _, y_mel = batch
                            y = y.to(device, non_blocking=True)
                            y_mel = torch.autograd.Variable(y_mel.to(device, non_blocking=True))
                            y_g_hat = generator(x.to(device))
                            y = scale_normalize(y.unsqueeze(1), y_g_hat).squeeze(1)
                            ...
                            if j <= 4:
                                if steps == 0:
                                    sw.add_audio('gt/y24_{}'.format(j), y[0], steps, h.sampling_rate)
                                    sw.add_figure('gt/y_spec_{}'.format(j), plot_spectrogram(x[0]), steps)

                                sw.add_audio('generated/y_hat24_{}'.format(j), y_g_hat[0], steps, h.sampling_rate)
...
def main():
    ...
    parser.add_argument('--input_wavs_dir', default='/hdd0/JIHOON/hyundai_data/hyundai_22050_unnorm_e5/train') # LJSpeech_22050 emotion_kor_jh
    parser.add_argument('--input_mels_dir', default='/hdd0/JIHOON/hyundai_data/hyundai_22050_unnorm_e5/train')
    parser.add_argument('--input_training_file', default='/hdd0/JIHOON/hyundai_data/hyundai_22050_unnorm_e5/train')
    parser.add_argument('--input_validation_file', default='/hdd0/JIHOON/hyundai_data/hyundai_22050_unnorm_e5/val')
    parser.add_argument('--checkpoint_path', default='/hdd0/JIHOON/hyundai_log/fre_hyundai_22050_unnorm_e5')
    parser.add_argument('--config', default='./config_v1_deeper.json')
